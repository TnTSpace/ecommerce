...
<div class="space-y-10 w-full max-w-6xl mx-auto pb-24">
	<!-- Header Section -->
	<header class="flex flex-col md:flex-row md:items-center justify-between gap-6 pt-6">
		<div class="flex items-center gap-5">
			<div
				class="p-4 bg-primary/10 rounded-2xl shadow-inner border border-primary/20 backdrop-blur-sm"
			>
				<Settings class="w-8 h-8 text-primary animate-spin-slow" />
			</div>
			<div>
				<h1 class="text-4xl font-bold tracking-tight text-slate-900 dark:text-white">Settings</h1>
	...
	<div class="grid grid-cols-1 lg:grid-cols-4 gap-10 items-start">
		<!-- Navigation Sidebar -->
		<aside
			class="lg:col-span-1 flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 no-scrollbar backdrop-blur-md bg-card/10 p-2 rounded-[2rem] border border-border/20 sticky top-24"
		>
			{#each tabs as tab}
				<button
					class={cn(
		...
		<main class="lg:col-span-3">
			{#if activeTab === 'account'}
				<div in:fly={{ y: 20, duration: 500 }} class="space-y-8">
					<!-- Profile Card -->
					<Card.Root
						class="bg-card/40 backdrop-blur-3xl border-border/40 rounded-[2.5rem] overflow-hidden shadow-2xl relative"
					>
						<div
						...
						<Card.Content class="p-8 md:p-12 space-y-10">
							<div class="flex flex-col md:flex-row items-center gap-8">
								<div class="relative group">
									<Avatar.Root
										class="w-32 h-32 border-4 border-background shadow-2xl rounded-[2.5rem] overflow-hidden group-hover:scale-[1.02] transition-transform duration-500"
									>
								...
								<div class="text-center md:text-left space-y-1">
									<h2 class="text-3xl font-bold">{userForm.name}</h2>
									<p
									...
									<Badge
										class="mt-2 bg-emerald-500/10 text-emerald-600 border-none font-bold capitalize text-sm tracking-widest px-3 py-1"
									>
										Verified Account
									</Badge>
								</div>
							...
							<div class="grid gap-8 pt-6 border-t border-border/20">
								<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
									<div class="space-y-3">
										...
										<Input
											...
											class="h-14 rounded-2xl bg-background/50 border-border/40 font-bold px-6 shadow-sm"
										/>
									</div>
									<div class="space-y-3">
										<Label
								...
								<div class="space-y-3">
									...
									<textarea
										...
										rows="4"
										class="w-full rounded-[1.5rem] bg-background/50 border border-border/40 p-6 font-medium text-base resize-none focus:ring-2 focus:ring-primary/20 outline-none transition-all shadow-sm"
									></textarea>
								</div>
							</div>
							...
							<div class="flex justify-end pt-4">
								<Button
									class="font-bold bg-primary shadow-xl shadow-primary/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
			...
			{:else if activeTab === 'notifications'}
				<div in:fly={{ y: 20, duration: 500 }} class="space-y-8">
					<Card.Root
						class="bg-card/40 backdrop-blur-3xl border-border/40 rounded-[2.5rem] overflow-hidden shadow-2xl"
					>
						...
						<Card.Content class="p-8 md:p-12 pt-0 space-y-4">
							<div class="grid gap-4">
								{#each [{ id: 'email', label: 'Email Reports', desc: 'Weekly health summaries and lab result alerts.' }, { id: 'push', label: 'On-Device Push', desc: 'Instant notifications for community tags and messages.' }, { id: 'sms', label: 'Priority SMS', desc: 'Emergency clinical alerts and appointment reminders.' }, { id: 'marketing', label: 'Health Intelligence', desc: 'Newsletter of medical masterclasses and research.' }] as item}
									<div
										class="flex items-center justify-between p-6 rounded-3xl bg-background/30 border border-border/40 hover:bg-background/50 transition-all group"
									>
										<div class="space-y-1">
							...
							<div
								class="p-6 bg-primary/5 rounded-3xl border border-primary/10 flex items-start gap-4 mt-8"
							>
					...
					>
						...
						<Card.Content class="p-8 md:p-12 pt-0 space-y-10">
								...
								<div class="grid gap-6">
									<div class="space-y-3">
										...
										<Input
											...
											class="h-14 rounded-2xl bg-background/50 border-border/40 px-6 font-bold"
										/>
									</div>
									<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
										<div class="space-y-3">
									...
									<Button
										class="w-full md:w-fit font-bold bg-primary shadow-xl shadow-primary/20"
							...
							<div class="pt-10 border-t border-border/20 space-y-6">
								...
								<div
									class="flex items-center justify-between p-8 rounded-[2rem] bg-indigo-500/5 border border-indigo-500/10"
								>
									<div class="flex items-center gap-6">
										<div class="p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm h-fit">
										...
										<div class="space-y-1">
											...
											<p class="text-sm text-indigo-900/40 dark:text-indigo-400/40 font-medium">
							...
							<div
								class="p-8 bg-destructive/5 border border-destructive/10 rounded-[2rem] space-y-6"
							>
