...
<div class="py-6 md:p-8 space-y-8 w-full overflow-x-hidden">
	<!-- Glassy Header -->
	<div
		class="flex flex-col md:flex-row md:items-center justify-between gap-6 p-8 rounded-3xl bg-card/40 backdrop-blur-xl border border-border/40 shadow-2xl relative overflow-hidden"
	>
		...
		<div class="flex items-center gap-4 relative z-10">
			<div
				class="px-6 py-3 rounded-2xl bg-primary/5 border border-primary/20 flex items-center gap-3 shadow-sm hover:bg-primary/10 transition-colors"
			>
				<div
					class="w-3 h-3 bg-orange-500 rounded-full animate-pulse shadow-lg shadow-orange-500/50"
				></div>
				<span class="font-bold text-primary tracking-tight">
					{#if $approvalsQuery.isSuccess}
						{$approvalsQuery.data.total} Applications Pending
					{:else}
						Calculating...
					{/if}
				</span>
	...
	<div class="relative max-w-2xl group">
		...
		<Input
			...
			class="h-14 pl-14 pr-6 rounded-2xl bg-card/40 border-border/40 backdrop-blur-md  shadow-lg focus:ring-primary/20 focus:border-primary/50 transition-all"
		/>
	</div>

	<Card
		class="border-border/40 shadow-2xl bg-card/40 backdrop-blur-md rounded-[2rem] overflow-hidden border-2"
	>
		<CardContent class="p-0">
					...
					<Table>
						<TableHeader>
							<TableRow class="bg-muted/50 hover:bg-muted/50 border-b border-border/40">
								<TableHead
									class="w-[300px] h-14 font-bold text-primary capitalize text-sm tracking-widest pl-8"
									>Practitioner Details</TableHead
								>
								<TableHead class="h-14 font-bold text-primary capitalize text-sm tracking-widest"
									>Specialty</TableHead
								>
								<TableHead
									class="h-14 font-bold text-primary capitalize text-sm tracking-widest text-center"
								...
								>
								<TableHead
									class="text-right h-14 font-bold text-primary capitalize text-sm tracking-widest pr-8"
									>Status</TableHead
								>
							</TableRow>
						</TableHeader>
									...
									<TableCell class="pl-8 py-5">
										<div class="grid grid-cols-[40px_1fr] items-center gap-4">
											<div class="relative">
												...
												{/if}
												<div
													class="absolute -bottom-1 -right-1 w-4 h-4 bg-orange-500 border-2 border-background rounded-full"
												></div>
											</div>
					...
					{#each $approvalsQuery.data.results as item}
						...
						<button
							class={cn(
								...
								'w-full text-left p-4 border border-border/40 bg-background/50 backdrop-blur-md shadow-lg active:scale-[0.98] transition-all space-y-4'
							)}
							onclick={() => openDetails(item)}
						>
									...
									<div class="relative">
										{#if d.image || u?.image}
											<img
												src={d.image || u?.image}
												alt={d.name}
												class="size-12 rounded-2xl object-cover border-2 border-background shadow-lg"
											/>
										{:else}
											<div
			...
			{#if $approvalsQuery.isSuccess && ($approvalsQuery.hasNextPage || $approvalsQuery.isFetchingNextPage)}
				<div class="p-8 flex justify-center border-t border-border/40">
					<Button
						variant="outline"
						class="border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary font-bold shadow-xl shadow-primary/5 group"
