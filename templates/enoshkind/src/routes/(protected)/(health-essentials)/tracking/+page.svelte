...
<div
	class="w-full min-h-screen pb-20 animate-in overflow-x-hidden bg-linear-to-b from-primary/5 via-background to-background"
>
	<!-- Top Navigation -->
	<div class="max-w-7xl mx-auto">
		...
		<div
			class="relative overflow-hidden bg-linear-to-r from-primary to-green-600 p-6 sm:p-8 text-white mb-8 shadow-2xl shadow-primary/20 group rounded-xl"
		>
			<div
			...
			<div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
				<div class="flex items-start gap-4">
					...
					<div>
						...
						<p class="text-white/80 text-sm font-medium max-w-md">
				...
				<div class="shrink-0">
					...
					<div class="relative">
						<Input
							...
							class="w-full sm:w-64 bg-white/10 backdrop-blur-md border border-white/30 h-12 text-white font-bold focus:ring-white/50 transition-all cursor-pointer"
						/>
					</div>
				</div>
			</div>
		...
		<Tabs.Root bind:value={activeTab} class="w-full">
			<Tabs.List
				class="flex p-1.5 bg-muted/30 backdrop-blur-md border border-border/50 mb-8 sticky top-20 z-40 max-w-2xl mx-auto shadow-sm"
			>
				{#each tabs as tab}
					{@const Icon = tab.icon}
					<Tabs.Trigger
						value={tab.id}
						class="relative flex-1 flex items-center justify-center gap-2.5 py-3 rounded-xl text-sm font-bold transition-all duration-300 z-10
			...
			<div class="max-w-4xl mx-auto space-y-8 pb-24">
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								...
								<div class="flex items-center gap-4">
									<div class="flex-1 space-y-2">
										<Input
											...
											class="w-full bg-muted/50 border-border/50 h-9 text-xl font-bold text-foreground focus:ring-primary/50 transition-all"
										/>
										<Label
											for="systolic"
							...
							>
								<div class="flex items-center justify-between mb-4">
									...
									<div
										class="p-2 bg-blue-500/10 rounded-xl group-hover:scale-110 transition-transform"
									>
								...
								<Input
									...
									class="w-full bg-muted/50 border-border/50 rounded-2xl h-9 text-2xl font-bold text-foreground focus:ring-blue-500/50 transition-all"
								/>
							</div>
							...
							>
								...
								<div class="flex items-center gap-6">
									<Input
										...
										class="w-full max-w-[200px] bg-muted/50 border-border/50 rounded-2xl h-9 text-2xl font-bold text-foreground focus:ring-red-500/50 transition-all"
									/>
						...
						<div class="space-y-10">
							...
							<div class="bg-card rounded-xl border border-border/50 p-8 shadow-sm">
								<div class="flex items-center justify-between mb-8">
									...
									<span class="text-3xl font-bold text-purple-600">{moodValue[0]}</span>
								</div>
								<div class="space-y-6">
									<Slider
										type="multiple"
							...
							<div class="bg-card rounded-xl border border-border/50 p-8 shadow-sm">
								<div class="flex items-center justify-between mb-8">
									...
									<span class="text-3xl font-bold text-green-600">{energyValue[0]}</span>
								</div>
								<div class="space-y-6">
									<Slider
										type="multiple"
							...
							<div class="bg-card rounded-xl border border-border/50 p-8 shadow-sm">
								...
								<Textarea
									...
									rows={4}
									class="w-full bg-muted/50 border-border/50 rounded-2xl px-6 py-4 text-base font-medium text-foreground focus:ring-primary/50 transition-all resize-none min-h-[120px]"
								></Textarea>
							</div>
						</div>
					</div>
				...
				{#if success}
					<div
						in:fade
						out:fade
						class="bg-green-500/10 border border-green-500/20 rounded-2xl p-4 flex items-center gap-3 text-green-600"
					>
				...
				{#if error}
					<div
						in:fade
						out:fade
						class="bg-destructive/10 border border-destructive/20 rounded-2xl p-4 flex items-center gap-3 text-destructive"
					>
				...
				<div class="pt-6">
					<Button
						...
						disabled={loading}
						class="w-full bg-linear-to-r from-primary to-green-600 text-white py-8 rounded-xl shadow-xl shadow-primary/30 hover:shadow-primary/50 transition-all hover:-translate-y-1 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 group"
					>
						{#if loading}
	...
	<div class="bg-card rounded-xl border border-border/50 p-6 shadow-sm group">
		...
		<div class="relative">
			<Input
				...
				class="w-full bg-muted/50 border-border/50 rounded-2xl h-9 text-xl font-bold {textColor} focus:ring-primary/50 transition-all"
			/>
		</div>
	</div>
{/snippet}
