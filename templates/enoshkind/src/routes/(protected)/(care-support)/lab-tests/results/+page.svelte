...
<div class="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
		{#if results.length === 0}
			<div
				class="flex flex-col items-center justify-center py-32 text-center space-y-6 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-slate-800 shadow-sm"
				in:fade
			>
				...
				<Button
					href="/lab-tests"
					class="rounded-xl h-12 px-8 font-bold shadow-lg shadow-primary/20"
				>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{#each filteredResults() as result, i}
					<div in:fly={{ y: 20, delay: i * 50 }}>
						<Card
							class="group hover:border-primary/20 transition-all duration-300 rounded-[2rem] overflow-hidden border-slate-200 dark:border-slate-800 h-full flex flex-col hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1 bg-white dark:bg-slate-900 shadow-sm"
						>
							<div class="p-6 sm:p-8 flex-1 flex flex-col space-y-6">
								...
								<div class="pt-4 mt-auto">
									<Button
										...
										class="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white hover:bg-primary hover:text-white rounded-xl h-12 font-bold transition-all group/btn flex items-center justify-between px-6"
									>
										View Analysis
										<ArrowRight
