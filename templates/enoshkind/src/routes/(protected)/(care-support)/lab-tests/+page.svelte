...
<div class="space-y-6 md:space-y-8 max-w-7xl mx-auto pb-12 overflow-x-hidden">
	...
	<div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pt-4">
		...
		>
			{#if isAdmin}
				<Button
					variant="outline"
					class="h-11 px-6 border-primary/20 hover:bg-primary/5 transition-all font-bold text-primary rounded-xl"
					onclick={triggerAdd}
				>
			...
			<Button
				variant="outline"
				class="h-11 px-6 border-slate-200 hover:bg-slate-50 transition-all font-bold rounded-xl"
			...
			<Button
				class="bg-primary hover:bg-primary/90 h-11 px-8 shadow-lg shadow-primary/20 font-bold rounded-xl transition-all active:scale-[0.98]"
	...
	<!-- DNA Banner -->
	<div
		class="relative overflow-hidden rounded-[2.5rem] bg-linear-to-br from-emerald-950 via-emerald-900 to-emerald-950 p-6 md:p-10 text-white shadow-2xl border border-emerald-500/10"
	>
		...
		>
			<div class="space-y-4 md:space-y-6 max-w-2xl text-center md:text-left">
				...
				<div class="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-4">
					<Button
						class="w-full sm:w-auto bg-white text-emerald-900 hover:bg-emerald-50 transition-all h-12 md:h-14 px-8 font-bold rounded-2xl text-base shadow-xl"
			...
			<div
				class="shrink-0 p-8 md:p-10 bg-emerald-500/10 rounded-[2.5rem] border border-emerald-500/20 backdrop-blur-xl shadow-inner group w-full md:w-auto flex justify-center"
			>
				<Dna
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
					...
					<div class="space-y-5">
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
							{#each category.tests as test}
								<Card.Root
									class="group hover:bg-variant-50 transition-all border-border/40 bg-card/40 backdrop-blur-sm overflow-hidden rounded-[2rem] shadow-sm hover:shadow-xl hover:-translate-y-1 duration-300 relative"
								>
		...
		<div class="space-y-8">
			<!-- Recent Results -->
			<Card.Root
				class="bg-card/40 border-border/40 rounded-[2.5rem] overflow-hidden shadow-sm backdrop-blur-sm"
			>
				<Card.Header class="pb-4 border-b border-border/20 px-6 sm:px-8">
					<Card.Title class="text-xl font-bold flex items-center justify-between">
						...
						<Button
							variant="ghost"
							size="icon"
							class="h-10 w-10 hover:bg-primary/10 hover:text-primary rounded-2xl transition-all"
						...
						>
							<Card.Content class="flex flex-col gap-4">
								<div class="flex items-center justify-between w-full">
									...
									<Badge
										class="shrink-0 bg-emerald-500/10 text-emerald-500 border-none text-[9px] font-bold capitalize tracking-tighter px-2 h-5"
			...
			<!-- Lab Info Process -->
			<Card.Root
				class="bg-primary/5 border-primary/10 overflow-hidden rounded-[2.5rem] shadow-sm border-2"
			>
