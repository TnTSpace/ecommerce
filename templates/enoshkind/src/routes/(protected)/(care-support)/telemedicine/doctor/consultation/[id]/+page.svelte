...
<div class="max-w-6xl mx-auto space-y-6 md:space-y-10">
	...
	<div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
		<Button
			...
			class="rounded-full h-10 w-10 sm:h-12 sm:w-12 p-0 border border-border/40 shrink-0"
		>
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
						...
						<div class="space-y-3 md:space-y-4 pt-2 md:pt-4">
							{#if session.status === 'pending'}
								...
								<form method="POST" action="?/accept" use:enhance>
									<Button
										class="w-full h-11 md:h-12 rounded-xl md:rounded-2xl font-bold text-sm md:text-base shadow-lg shadow-primary/30 gap-2"
									>
							...
							{:else if session.status === 'confirmed'}
								...
								<Button
									...
									class="w-full h-11 md:h-12 rounded-xl md:rounded-2xl font-bold text-sm md:text-base bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-500/30 gap-2"
								>
							...
							{:else if session.status === 'completed'}
								...
								{#if session.payoutStatus === 'none'}
									<Button
										...
										class="w-full h-11 md:h-12 rounded-xl md:rounded-2xl font-bold text-sm md:text-base bg-green-600 hover:bg-green-700 shadow-lg shadow-green-500/30 gap-2"
									>
										{#if initiatingPayout}
		...
		<div class="lg:col-span-2">
			<Card
				class="border-border/40 shadow-xl rounded-2xl md:rounded-[2.5rem] overflow-hidden min-h-[400px] md:min-h-[600px]"
			>
					...
					{:else}
						...
						>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
								<div class="space-y-2 md:space-y-3">
									...
									<Input
										...
										class="h-10 md:h-12 rounded-xl md:rounded-2xl bg-background border-border/40 px-4 md:px-6 font-medium shadow-inner text-sm"
									/>
								</div>
								<div class="space-y-2 md:space-y-3">
							...
							<div class="pt-4 md:pt-6 border-t border-border/40 flex justify-end">
								<Button
									type="submit"
									class="w-full sm:w-auto h-12 md:h-14 px-6 md:px-10 rounded-xl md:rounded-2xl font-bold text-sm md: bg-primary shadow-lg shadow-primary/30 group gap-2 md:gap-3"
								>
