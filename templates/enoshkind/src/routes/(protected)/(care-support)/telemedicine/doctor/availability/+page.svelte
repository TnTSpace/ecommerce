...
<div class="min-h-screen bg-slate-50/50 dark:bg-slate-950/50">
	<div class="max-w-5xl mx-auto py-10 lg:py-16">
		<div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
			<div class="space-y-3">
				<div
					class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-sm font-bold capitalize tracking-widest"
				>
					<Clock class="w-4 h-4" />
					Practice Management
				</div>
			...
			<Button
				...
				disabled={saving}
				class="h-9 px-10  gap-3 shadow-xl shadow-primary/20 hover:shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all group shrink-0"
			>
				{#if saving}
		...
		{#if loading}
			<div class="flex flex-col items-center justify-center py-32 gap-6 animate-in fade-in">
				<div class="relative">
					...
					<div
						class="absolute -top-2 -right-2 w-6 h-6 bg-primary rounded-full animate-ping opacity-50"
					></div>
				</div>
		...
		{:else}
			<div class="grid gap-8">
				{#each days as day, i}
					...
					<Card
						class="border-border/40 shadow-xl overflow-hidden bg-background/80 backdrop-blur-xl group hover:shadow-2xl transition-all duration-500"
					>
						<CardHeader
						...
						>
							...
							<Button
								variant="outline"
								size="sm"
								class="h-10 px-5 gap-2 bg-background border-primary/20 text-primary hover:bg-primary hover:text-white font-bold transition-all shadow-sm w-full sm:w-auto"
								onclick={() => addSlot(i)}
							>
								<Plus class="w-4 h-4" />
								New Slot
						...
						<CardContent>
							{#if daySlots.length === 0}
								<div class="text-center py-12 border-2 border-dashed border-border/20 bg-muted/5">
							...
							{:else}
								<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
									{#each availability as slot, idx}
										{#if slot.dayOfWeek === i}
											<div
												class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 animate-in fade-in zoom-in-95 duration-300 bg-muted/30 sm:bg-transparent p-5 sm:p-0 rounded-[2rem] border border-border/20 sm:border-none"
											>
												<div
													class="flex-1 flex flex-col sm:flex-row items-center justify-center gap-4 bg-background border-2 border-border/40 p-4 sm:p-5 shadow-inner focus-within:border-primary/50 focus-within:ring-4 focus-within:ring-primary/5 transition-all group/input"
												>
													<div
													...
													>
														<div class="flex items-center gap-3 w-full sm:w-auto px-2">
															<Clock
																class="w-5 h-5 text-muted-foreground group-focus-within/input:text-primary transition-colors"
															/>
															<div class="flex flex-col">
																<span
																	class="text-sm capitalize font-bold text-muted-foreground/60 sm:hidden"
																	>Start Time</span
																>
																<input
																	type="time"
																	bind:value={slot.startTime}
																	class="bg-transparent border-none outline-none text-2xl sm:text-xl font-bold w-full sm:w-24 text-center sm:text-left cursor-pointer"
																/>
															</div>
														</div>
														<div
															class="hidden sm:block w-6 h-0.5 bg-muted-foreground/30 rounded-full"
														></div>
														<div class="w-full h-px bg-border/40 sm:hidden my-1"></div>
														<div class="flex items-center gap-3 w-full sm:w-auto px-2">
												...
												<Button
													variant="ghost"
													size="icon"
													class="h-9 sm:h-16 w-full sm:w-16 text-destructive hover:bg-destructive/10 transition-all shrink-0 flex items-center justify-center gap-2 sm:gap-0 font-bold"
