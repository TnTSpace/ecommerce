		...
		try {
			const response = await fetch('/api/pharmacy-orders', {
				...
				body: JSON.stringify({
					...
					totalAmount: 450000 // Mock price for demo (4,500 NGN)
				})
			});
...
<Dialog.Root bind:open>
	<Dialog.Content
		class="sm:max-w-[450px] rounded-[2rem] border-border/40 bg-card/60 backdrop-blur-2xl shadow-2xl"
	>
		<Dialog.Header>
			...
			<div class="space-y-4">
				...
				>
					...
					<div>
						...
						<p class="text-sm text-muted-foreground">Partnered with NAFDAC certified outlets</p>
					</div>
				</div>
			</div>
