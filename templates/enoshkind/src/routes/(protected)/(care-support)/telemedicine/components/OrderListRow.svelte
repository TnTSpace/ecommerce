			...
			<div class="flex items-center justify-end gap-2">
				{#if type === 'consultation' && item.paid}
					<Button
						...
						class="h-8 rounded-xl font-bold gap-1.5"
					>
...
{:else}
	<div
		class="bg-card/40 backdrop-blur-xl border border-border/50 rounded-2xl p-4 space-y-4 shadow-lg group hover:border-primary/40 transition-all"
	>
		...
		<div class="grid grid-cols-2 gap-2 p-2 bg-muted/20 rounded-xl">
			...
			<div>
				...
				<Badge
					variant={item.paid ? 'default' : 'destructive'}
					class="font-bold text-[9px] px-1.5 py-0 h-4 rounded-full"
				>
		...
		<div class="flex gap-2">
			{#if type === 'consultation' && item.paid}
				<Button
					href="/telemedicine/{item.id}/stream"
					class="flex-1 rounded-xl font-bold h-10 shadow-lg"
				>
			...
			{:else}
				<Button
					variant="outline"
					class="flex-1 rounded-xl font-bold h-10 bg-white"
					onclick={() => onViewDetails?.(item, type)}
				>
					Details
				</Button>
