...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
	...
	<div class="relative h-[400px] rounded-[3rem] overflow-hidden shadow-2xl border border-border/50">
		...
		>
			<div class="max-w-2xl space-y-4 relative z-10">
				<Badge
					class="bg-primary/20 hover:bg-primary/30 text-primary border-primary/20 backdrop-blur-md px-4 py-1.5 font-bold"
				>
				...
				<div class="flex flex-col md:flex-row gap-4 pt-4">
					<div class="relative flex-1">
						...
						<input
							...
							class="w-full h-9 pl-12 pr-4 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all shadow-lg"
						/>
					</div>
					<Button
						class="h-9 px-8 bg-primary hover:bg-primary/90 font-bold  shadow-xl shadow-primary/20"
					>
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
		...
		<div class="lg:col-span-1 space-y-6">
			<Card.Root
				class="bg-card/40 backdrop-blur-xl border-border/50 rounded-[2.5rem] overflow-hidden shadow-xl border-2"
			>
				<Card.Header>
					...
					<div class="space-y-4">
						{#each [{ icon: Truck, title: 'Rapid Delivery', desc: 'Avg. 45 min delivery time' }, { icon: Clock, title: '24/7 Access', desc: 'Critical care anytime' }, { icon: Store, title: 'Click & Collect', desc: 'Skip the pharmaceutical queue' }] as feature}
							<div class="flex items-center gap-4 group cursor-help">
								<div
									class="w-10 h-10 bg-muted flex items-center justify-center shrink-0 group-hover:bg-primary/10 transition-colors"
								>
									<feature.icon
			...
			<div
				class="bg-gradient-to-br from-indigo-600 to-indigo-700/60 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl group cursor-pointer"
			>
				<div
					class="absolute -top-12 -right-12 w-48 h-48 bg-white/10 rounded-full blur-3xl transform group-hover:scale-110 transition-transform"
				></div>
				...
				<Button class="mt-6 bg-white text-indigo-700 hover:bg-white/90 px-6 border-none">
					Enable Now
		...
		<div class="lg:col-span-2 space-y-6">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				{#each filteredPharmacies as pharmacy}
					<Card.Root
						class="group hover:shadow-2xl transition-all duration-500 border-border/40 hover:border-primary/40 overflow-hidden bg-card/40 backdrop-blur-xl rounded-[2rem]"
					>
							...
							<div class="pt-4 border-t border-border/40 flex items-center justify-between">
								...
								<div class="flex gap-2">
									<Button
										variant="ghost"
										size="icon"
										class="hover:bg-primary/10 hover:text-primary transition-all h-10 w-10"
									>
										<Phone class="w-4 h-4" />
									</Button>
									<Button
										class="font-bold bg-primary hover:bg-primary/90 h-10 px-4 shadow-lg shadow-primary/10"
			...
			<!-- Bottom CTA -->
			<div
				class="p-8 rounded-[2rem] bg-card/40 border border-border/50 text-center space-y-4 relative overflow-hidden group"
			>
				<div
