						...
						{#if item.roles.includes(user.role as Role)}
							<a class="cursor-pointer" href={item.url}>
								<Sidebar.MenuItem>
									<Sidebar.MenuButton
										class={cn(
											'cursor-pointer justify-start transition-all duration-300 h-11 px-4 rounded-[14px] font-bold gap-3',
											item.isActive
												? 'bg-green-500 text-white shadow-lg shadow-green-500/20 hover:bg-green-600 hover:text-white scale-[1.02]'
