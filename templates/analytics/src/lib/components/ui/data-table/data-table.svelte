<script lang="ts" generics="TData, TValue">
	...
	import Country from '$lib/components/icons/countries/Country.svelte';
...
{#if mounted}
	<div class="bg-white dark:bg-secondary rounded-lg p-2 flex flex-col gap-2">
		<div class="grid grid-cols-1 items-center gap-4 md:grid-cols-[1fr_2fr] lg:grid-cols-[1fr_4fr_1fr]">
			<div class="flex items-center gap-2">
				<Country>
					<div class="flex text-muted-foreground text-xs">
						...
						<Table.Row>
							{#each headerGroup.headers as header, i}
								{#if i === headerGroup.headers.length - 1}
									<Table.Head class="justify-end text-right bg-background">
								...
								{:else if i === 0}
									<Table.Head class="items-center gap-1 bg-background">
										{#if !header.isPlaceholder}
								...
								{:else}
									<Table.Head class="bg-background">
										{#if !header.isPlaceholder}
											<FlexRender
												content={header.column.columnDef.header}
												context={header.getContext()}
					...
					{#each table.getRowModel().rows as row (row.id)}
						...
						<Table.Row class="relative align-middle" data-state={row.getIsSelected() && 'selected'}>
							{#each row.getVisibleCells() as cell, i}
								{#if i === 0}
									<Table.Cell class="w-10 bg-background">
								...
								{:else if i === row.getVisibleCells().length - 1}
									<Table.Cell class="justify-end text-right bg-background">
										<FlexRender content={cell.column.columnDef.cell} context={cell.getContext()} />
									</Table.Cell>
								{:else}
									<Table.Cell class="bg-background">
										<FlexRender content={cell.column.columnDef.cell} context={cell.getContext()} />
									</Table.Cell>
								{/if}
							{/each}
