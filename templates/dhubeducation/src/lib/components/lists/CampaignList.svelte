<script lang="ts"> 
	...
	import CampaignInfiniteList from './CampaignInfiniteList.svelte';
	...
	let { campaigns: items }: Props = $props()

	const filterKeys: (keyof iCampaign)[] = ['title', 'xata_createdat', 'xata_updatedat', 'description', 'slug', 'content'];

	let itemsStore: Writable<iCampaign[]> = writable(items);
	...
	const href = `/admin/campaigns/new`;
</script>

<div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
	<Heading
		title={`Campaigns (${$itemsStore.length})`}
...
{#key $itemsStore.length}
	<CampaignInfiniteList list={$itemsStore} class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4" />
{/key}
