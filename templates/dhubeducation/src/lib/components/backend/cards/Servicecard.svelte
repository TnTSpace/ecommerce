<script lang="ts">
	import type { iFile, iService, TService } from '$lib/interface';
	...
	let { service, onedit }: Props = $props();

	const { xata_id } = service;

	const style = getStyle((service.file as iFile)?.url || '/placeholder.webp');
</script>
...
>
	...
	<div class="flex flex-col items-center space-y-1 p-4 pt-0">
		...
		<p class="w-full overflow-hidden text-ellipsis whitespace-pre-wrap text-muted-foreground">
			{service.description}
		</p>
	</div>
